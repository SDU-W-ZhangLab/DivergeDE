...
def fit_null(
    ...
    loglik = float(np.sum(nb_logpmf(y, mu, r)))
    at_lower_boundary = r <= R_MIN * (1.0 + 1e-6)
    converged = bool(result.success and np.isfinite(loglik) and not at_lower_boundary)
    message = (
        "r reached its lower numerical boundary"
...
def fit_alternative_start(
    ...
    r_at_lower_boundary = r <= R_MIN * (1.0 + 1e-6)
    if delta_at_boundary or r_at_lower_boundary:
        converged = False
        message = (
            "a branch effect reached its numerical boundary"
            if delta_at_boundary
            else "r reached its lower numerical boundary"
        )
